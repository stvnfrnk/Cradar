...
def coords2shape(X, Y, Frame, Segment, Season, EPSG_in=4326, EPSG_out=4326, geometry='Point', step=1, attributes=[]):
    ...
    df['Segment'] = Segment
    df['Season']  = Season
    df.columns    = ['X', 'Y', 'Frame', 'Segment', 'Season']
    ...
    if geometry == 'Line':
        ...
        gdf_line = gdf_point.groupby(['Frame', 'Segment', 'Season'])['geometry'].apply(lambda x: LineString(x.tolist()))
        gdf_line = gpd.GeoDataFrame(gdf_line, crs=EPSG_out, geometry='geometry')
...
def gridtrack(Longitude='', Latitude='', geotif='', geotif_name='', geotif_epsg=''):
    ...
    transformer = Transformer.from_crs(4326, geotif_epsg, always_xy=True)
